...
const count = (target, input) => {
  return input.reduce((total, array) => {
    const occurrencesInArray = array.filter(item => item === target).length;
    return total + occurrencesInArray;
  }, 0);
};
...
let findMaleAndFemale = (data) => {
  ...
  return maleAndFemaleCharacters;
};
...
let findShortest = (data) => {
  ...
  return shortestCharacter ? shortestCharacter.name : null;
};
